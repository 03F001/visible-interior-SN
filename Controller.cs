...
namespace VisibleLockerInterior {
	internal class Controller {
		...
		private static readonly float[] xPos = { 0.469f  , 0.335f  , 0.201f   , 0.067f  , -0.067f   , -0.201f   , -0.335f, -0.469f };
		private static readonly float[] yPos = { 1.58803f, 1.32938f, 0.966707f, 0.57335f,  0.288304f,  0.046258f };
		...
		public static void UpdateInterior(StorageContainer sc) {
			...
			for (int i = 0, j = 0; i < items.Count || j < dummies.Count;) {
				var row = i / itemPerRow;
				var col = i % itemPerRow;
				if (col >= xPos.Length || row >= yPos.Length) break;
				var targetPosition = new Vector3(xPos[col], yPos[row], zPos);
				int cmp = 
					i == items.Count ? 1 :
					j == dummies.Count ? -1 :
					CompareTechType(
